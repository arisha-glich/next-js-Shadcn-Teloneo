  ...
  cell: ({ row }) => {
   ...
   return (
    <div className="flex items-center space-x-6">
     ...
     <div className="flex items-center">
      <Switch
       checked={original.isActive} // Use the original value from the row data
       onCheckedChange={() => {
        console.log('Toggled active state for:', original.name);
        // Call a function here to handle the state update at a higher level
       }}
       className={`cursor-pointer ${original.isActive ? 'text-primary' : 'text-gray-500'}`}
      />
      <span className="ml-2">{original.isActive ? 'Active' : 'Inactive'}</span>
     </div>
    </div>
   );
  },
